...
This is the most basic test case for the emitter–receiver geometry: the
injected TE10 mode propagates as a clean guided mode with no radiation
losses, diffraction, or geometric transitions.  Use it to validate the
mode injection, port diagnostics, and S21 reconstruction before adding
the horn geometry or scooter section.
"""
...
t_sim_override = None     # s  (None = auto)
...
# ---- Grid resolution ----
cells_per_wavelength = 64    # higher resolution for this validation case
cfl = 0.995
...
# PEC in X/Y (Dirichlet = perfect conductor) replaces the need for an
# embedded boundary.  PML in Z absorbs the outgoing guided wave at both
# ends so no energy is trapped or reflected back into the domain.
