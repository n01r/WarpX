...
- The straight-guide buffer on each side of the diagnostic planes is long
  enough that the direct pulse and its first reflection off the PML are
  temporally separated at both injection and receiving planes.  The
  positions of inject_z / recv_z (and therefore the waveguide length seen
  by S21) are unchanged compared with the earlier short-box version.
...
# ---- Z-domain buffers ----
# Keep enough straight guide between the source/receiver planes and the PML
# that the direct pulse and the first PML reflection do *not* overlap in
# time at either diagnostic plane.
#
# Scaling: the Gaussian pulse has tau = FWHM / (2*sqrt(ln 2)) ~= 30 ps, so
# its 6-sigma duration is ~180 ps.  At 67 GHz the TE10 group velocity is
# vg ~= 0.80 c ~= 2.4e8 m/s, so a 50 mm buffer gives a round-trip
# 2 * buffer / vg ~= 420 ps -- comfortably more than the pulse duration
# (and ~2.5x margin), so the reflection at each plane arrives well after
# the direct pulse has passed.  A shorter 10 mm buffer (previous value)
# gave a round-trip of only ~85 ps, which caused severe overlap.
#
# inject_z and recv_z themselves are *not* changed, so the waveguide
# length L = |recv_z - inject_z| seen by S21 matches earlier cases.
z_buffer_before_emit = 50e-3   # m
z_buffer_after_recv  = 50e-3   # m

# ---- PML ----
# 32 cells (doubled from the original 16) for a thicker absorbing layer.
# For a TE10 mode that enters the PML obliquely in its frequency-dependent
# group velocity, doubling the PML thickness drops the residual reflection
# roughly by an order of magnitude with only a small additional cost in
# memory/compute because the PML is Z-only in this setup.
pml_ncells = 32
...
# Approximate time between arrival of the direct pulse at each diagnostic
# plane and the first return of a PML reflection at that same plane.  Only
# valid when the PML is the dominant reflector (it is, in this setup).
t_echo_inject = 2 * (inject_z - z_lo) / vg
t_echo_recv   = 2 * (z_hi - recv_z) / vg
...
print(f"Injection  z = {inject_z*1e2:.5f} cm  (buffer to -z PML = {(inject_z - z_lo)*1e3:.2f} mm)")
print(f"Receiving  z = {recv_z*1e2:.5f} cm  (buffer to +z PML = {(z_hi - recv_z)*1e3:.2f} mm)")
print(f"Propagation: {emitter_to_receiver*1e2:.3f} cm ({t_propagation*1e9:.3f} ns)")
if excitation_mode == "pulse":
    print(
        f"Echo separation (round-trip to PML): inject {t_echo_inject*1e12:.0f} ps, "
        f"recv {t_echo_recv*1e12:.0f} ps  | pulse 6-sigma ~= {6*pulse_tau*1e12:.0f} ps"
    )
if excitation_mode == "cw":
