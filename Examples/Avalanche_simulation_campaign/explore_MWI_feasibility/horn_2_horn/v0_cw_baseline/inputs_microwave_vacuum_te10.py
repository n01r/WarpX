...
lasy_laser = Laser(
    dim='xyt',
    lo=[-(a_horn/2+dx), -(b_horn/2+dy), 0.0],
    hi=[ (a_horn/2+dx),  (b_horn/2+dy), laser_t_max],
...
recv_plane_probe = picmi.ReducedDiagnostic(
    ...
    detector_radius=min(horn_half_width_x, horn_half_width_y),  # Half edge length # Should be max(), actually ... sampling zeros is okay
    target_normal_x=0.0,
    target_normal_y=0.0,
